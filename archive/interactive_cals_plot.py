...
class InteractiveCalsPlot:
    def __init__(self):
        # Read the CSV file containing caloric macros
        self.df = pd.read_csv('cals.csv')

        # Parse the DateTime column
        self.df['DateTime'] = pd.to_datetime(self.df['DateTime'])

        # Ignore the first datapoint (header row) if necessary (matching weight script behavior)
        self.df = self.df.iloc[1:].reset_index(drop=True)

        # Ensure we have proper datetime index
        self.df = self.df.set_index('DateTime')

        # Calculate total calories per day by summing macros
        self.df['Total Calories'] = self.df[['Fat', 'Carbs', 'Protein']].sum(axis=1)

        # Add week number starting from Monday
        self.df['Week'] = self.df.index.to_period('W-MON')

        # Calculate weekly averages of total calories
        self.weekly_avg = self.df.groupby('Week')['Total Calories'].mean()
    ...
    def setup_plot(self):
        ...
        self.weekly_scatter = self.ax.scatter(
            self.week_starts,
            self.weekly_avg.values,
            c=weekly_colors,
            s=60,
            zorder=5,
        ...
        # Format the plot
        self.ax.set_title('Calorie Tracking with Interactive Trend Line', fontsize=14, fontweight='bold')
        self.ax.set_xlabel('Date', fontsize=12)
        self.ax.set_ylabel('Calories', fontsize=12)
        self.ax.legend()
        ...
        # Dragging disabled as in the weight plot
    ...
    def on_click(self, event):
        ...
        if event.button == 1:  # Left click
            ...
            if self.trend_start_idx is None:
                ...
                print(
                    f"Trend line start set to Week {closest_idx}: "
                    f"{self.weekly_avg.index[closest_idx]} → {self.weekly_avg.iloc[closest_idx]:.2f} kcal"
                )
                ...
                self.highlight_point(closest_idx, 'green')
                print(
                    f"Trend line end set to Week {closest_idx}: "
                    f"{self.weekly_avg.index[closest_idx]} → {self.weekly_avg.iloc[closest_idx]:.2f} kcal"
                )
                ...
                self.highlight_point(closest_idx, 'green')
                print(
                    f"Reset: Trend line start set to Week {closest_idx}: "
                    f"{self.weekly_avg.index[closest_idx]} → {self.weekly_avg.iloc[closest_idx]:.2f} kcal"
                )
    ...
    def draw_trend_line(self):
        ...
        start_cals = self.weekly_avg.iloc[self.trend_start_idx]
        end_cals = self.weekly_avg.iloc[self.trend_end_idx]

        # Calculate statistics
        num_weeks = self.trend_end_idx - self.trend_start_idx
        weekly_change = (end_cals - start_cals) / num_weeks if num_weeks > 0 else 0
        ...
        self.trend_line, = self.ax.plot(
            ...
            linewidth=3,
            alpha=0.8,
            label=f'Trend Line ({num_weeks} weeks)'
        )

        # Add statistics text at fixed position (top right of plot)
        stats_text = (
            f"Start: {start_cals:.2f} kcal\n"
            f"End: {end_cals:.2f} kcal\n"
            f"Change: {end_cals - start_cals:.2f} kcal\n"
            f"Weekly: {weekly_change:.2f} kcal/week"
        )
        ...
        self.print_trend_stats(start_week, end_week, start_cals, end_cals, weekly_change, num_weeks)
    ...
    def clear_trend_line(self):
        ...
        # Reset all point colors to canonical week colors
        self.reset_point_colors()

    def print_trend_stats(self, start_week, end_week, start_cals, end_cals, weekly_change, num_weeks):
        """Print detailed trend statistics to console"""
        print(f"\n{'='*60}")
        print("CALORIE TREND LINE ANALYSIS")
        print(f"{'='*60}")
        print(f"Start Week:      {start_week}")
        print(f"Starting Calories: {start_cals:.2f} kcal")
        print(f"End Week:        {end_week}")
        print(f"Ending Calories:   {end_cals:.2f} kcal")
        print(f"Total Change:    {end_cals - start_cals:.2f} kcal")
        print(f"Weekly Change:   {weekly_change:.2f} kcal/week")
        print(f"Duration:        {num_weeks} weeks")
        print(f"{'='*60}\n")


def main():
    print("Starting Interactive Calorie Plot...")
    print("Click on weekly average points to create trend lines!")

    # Create and show the interactive plot
    plot = InteractiveCalsPlot()
